...
class ReportUtilsTests(TestCase):
    ...
    def test_check_report_view_permission(self):
        ...
        # Probar permisos
        self.assertIsNone(_utils.check_report_view_permission(admin_request, self.user_report))
        self.assertIsNone(_utils.check_report_view_permission(owner_request, self.user_report))
        self.assertIsInstance(_utils.check_report_view_permission(other_request, self.user_report), HttpResponseForbidden)
    ...
    def test_check_report_edit_permission(self, mock_messages):
        ...
        # Probar permisos
        self.assertIsNone(_utils.check_report_edit_permission(admin_request, self.user_report))
        self.assertIsNone(_utils.check_report_edit_permission(owner_request, self.user_report))
        self.assertIsInstance(_utils.check_report_edit_permission(other_request, self.user_report), HttpResponseForbidden)
        ...
        with patch('reports._utils.redirect') as mock_redirect:
            ...
            result = _utils.check_report_edit_permission(owner_request, self.user_report)
    ...
    def test_filter_reports(self):
        ...
        filtered, _ = _utils.filter_reports(request, all_reports)
        self.assertEqual(filtered.count(), 1)
        self.assertTrue('urgente' in filtered.first().title.lower() or 'urgente' in filtered.first().description.lower())
    ...
    def test_paginate_reports(self):
        ...
        page_obj = _utils.paginate_reports(request, all_reports)
        self.assertEqual(len(page_obj), 6)  # 16 reportes en total, 6 en la segunda página
