...
from .constants import (IMPORTANT_IMPORTANCE, NORMAL_IMPORTANCE,
                        PAYMENT_REPORT, RIDE_REPORT, SYSTEM_REPORT,
                        URGENT_IMPORTANCE, USER_REPORT)
...
def get_reports_with_response():
    ...
    """
    return (
        Report.objects.exclude(response__isnull=True)
        .exclude(response="")
        .order_by("-created_at")
    )
...
def get_reports_without_response():
    ...
    """
    return Report.objects.filter(Q(response__isnull=True) | Q(response="")).order_by(
        "-created_at"
    )
...
def create_user_report(
    user, reported_user, title, description, importance=NORMAL_IMPORTANCE
):
    """
...
def create_payment_report(
    user, payment, title, description, importance=NORMAL_IMPORTANCE
):
    """
...
def get_daily_report_counts(reports, start_date, end_date):
    ...
    day_counts = (
        reports.annotate(date=TruncDate("created_at"))
        .values("date")
...
def get_hourly_report_counts(reports, date):
    ...
    hour_counts = (
        reports.filter(created_at__date=date)
