...
class ReviewEndpointSecurityTests(TestCase):
    ...
    def test_create_review_for_future_ride(self):
        ...
        response = self.client.get(
            reverse(URL_CREATE_REVIEW, args=[self.future_ride.id])
        )
        ...
        self.assertFalse(
            Review.objects.filter(ride=self.future_ride, user=self.passenger).exists()
        )
    ...
    def test_create_review_by_uninvolved_user(self):
        ...
        # Verificar que no se creó la valoración
        self.assertFalse(
            Review.objects.filter(ride=self.past_ride, user=self.other_user).exists()
        )
    ...
    def test_create_duplicate_review(self):
        ...
        form_data = {"rating": 5, "comment": "Otro comentario"}
        response = self.client.post(
            reverse(URL_CREATE_REVIEW, args=[self.past_ride.id]), form_data
        )
        ...
        self.assertEqual(
            Review.objects.filter(ride=self.past_ride, user=self.passenger).count(), 1
        )
    ...
    def test_csrf_protection_for_create_review(self):
        ...
        form_data = {"rating": 5, "comment": "Gran experiencia con el pasajero"}
        response = self.client.post(
            reverse(URL_CREATE_REVIEW, args=[self.past_ride.id]), form_data
        )
    ...
    def test_access_nonexistent_review(self):
        """
        Prueba acceder a una valoración que no existe.
        """
        print("\nProbando acceso a valoración inexistente...")

        # Login como usuario cualquiera
        self.client.login(username=DRIVER_USERNAME, password=DRIVER_PASSWORD)

        # Intentar acceder a un detalle con ID que no existe
        response = self.client.get(reverse(URL_DETAIL_REVIEW, args=[9999]))
    ...
    def test_delete_nonexistent_review(self):
        """
        Prueba eliminar una valoración que no existe.
        """
        print("\nProbando eliminar valoración inexistente...")

        # Login como usuario cualquiera
        self.client.login(username=PASSENGER_USERNAME, password=PASSENGER_PASSWORD)

        # Intentar eliminar una valoración con ID que no existe
        response = self.client.post(reverse(URL_DELETE_REVIEW, args=[9999]))

        # Debe devolver error 404
        self.assertEqual(response.status_code, 404)

    def test_rate_ride_with_invalid_data(self):
        """
        Prueba crear una valoración con datos inválidos.
        """
        print("\nProbando valoración con datos inválidos...")

        # Login como conductor (que no ha valorado aún)
        self.client.login(username=DRIVER_USERNAME, password=DRIVER_PASSWORD)

        # Intentar crear una valoración con rating fuera de rango
        form_data = {"rating": 10, "comment": "Buen viaje"}  # Fuera del rango permitido
        response = self.client.post(
            reverse(URL_CREATE_REVIEW, args=[self.past_ride.id]), form_data
        )

        # Debería rechazar el formulario y mostrar errores
