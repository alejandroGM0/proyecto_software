...
class ReviewViewsTests(TestCase):
    ...
    def test_create_review_view_post(self):
        ...
        response = self.client.post(
            reverse(URL_CREATE_REVIEW, args=[new_past_ride.id]),
            {"rating": REVIEW_RATING, "comment": REVIEW_COMMENT},
        )

        # Verificar redirección y mensaje
        self.assertRedirects(
            response, reverse("rides:ride_detail", args=[new_past_ride.id])
        )

        # Verificar que se creó la valoración
        self.assertTrue(
            Review.objects.filter(ride=new_past_ride, user=self.passenger).exists()
        )
    ...
    def test_create_review_future_ride(self):
        ...
        self.assertRedirects(
            response, reverse("rides:ride_detail", args=[self.future_ride.id])
        )
    ...
    def test_create_review_already_reviewed(self):
        ...
        response = self.client.post(
            reverse(URL_CREATE_REVIEW, args=[self.past_ride.id]),
            {"rating": REVIEW_RATING, "comment": REVIEW_COMMENT},
        )

        # Debería redirigir con un mensaje de error
        self.assertRedirects(
            response, reverse("rides:ride_detail", args=[self.past_ride.id])
        )
