...
class ReviewPublicAPITests(TestCase):
    ...
    def test_get_user_already_reviewed(self):
        ...
        already_reviewed = public.get_user_already_reviewed(
            self.passenger, self.past_ride
        )
    ...
    def test_get_user_average_rating(self):
        ...
        Review.objects.create(
            ride=new_ride, user=self.passenger, rating=3, comment="Viaje normal"
        )
    ...
    def test_user_has_participation(self):
        ...
        has_participation = public.user_has_participation(
            self.passenger, self.past_ride
        )
        self.assertTrue(has_participation)
        ...
        has_participation = public.user_has_participation(
            self.other_user, self.past_ride
        )
    ...
    def test_create_review(self):
        ...
        review = public.create_review(
            user=self.driver, ride=self.past_ride, rating=5, comment="Gran experiencia"
        )
        ...
        self.assertIsNotNone(
            review, "La valoración con rating ajustado no debería ser None"
        )
        if review:  # Verificamos solo si review no es None
