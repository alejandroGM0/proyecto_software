...
class PaymentUtilsTests(TestCase):
    ...
    def setUp(self):
        ...
        self.payer_profile = UserProfile.objects.create(
            user=self.payer, has_payment_method=False
        )
        ...
        self.recipient_profile = UserProfile.objects.create(
            user=self.recipient, stripe_account_id="acct_test_12345"
        )
    ...
    def test_create_checkout_session(self, mock_create):
        ...
        self.assertEqual(
            self.pending_payment.stripe_payment_intent_id, "test_checkout_session_id"
        )
    ...
    def test_process_refund(self, mock_refund_create, mock_session_retrieve):
        ...
        mock_refund_create.assert_called_once_with(
            payment_intent="pi_test_12345", reason="requested_by_customer"
        )
    ...
    def test_validate_payment_access(self):
        ...
        self.assertTrue(
            _utils.validate_payment_access(self.pending_payment, self.payer)
        )

        # El receptor tiene acceso
        self.assertTrue(
            _utils.validate_payment_access(self.pending_payment, self.recipient)
        )

        # Un tercero no tiene acceso
        other_user = User.objects.create_user("otro", "otro@example.com", "password123")
        self.assertFalse(
            _utils.validate_payment_access(self.pending_payment, other_user)
        )
    ...
    def test_validate_refund_permission(self):
        ...
        has_perm, _ = _utils.validate_refund_permission(
            self.completed_payment, self.recipient
        )
        self.assertTrue(has_perm)

        # El pagador no puede reembolsar
        has_perm, error_msg = _utils.validate_refund_permission(
            self.completed_payment, self.payer
        )
        ...
        has_perm, error_msg = _utils.validate_refund_permission(
            self.pending_payment, self.recipient
        )
        self.assertFalse(has_perm)
    ...
    def test_validate_cancel_permission(self):
        ...
        has_perm, _ = _utils.validate_cancel_permission(
            self.pending_payment, self.payer
        )
        self.assertTrue(has_perm)

        # El receptor no puede cancelar
        has_perm, error_msg = _utils.validate_cancel_permission(
            self.pending_payment, self.recipient
        )
        self.assertFalse(has_perm)
