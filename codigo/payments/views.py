...
from . import _utils
from .constants import (ERROR_ALREADY_PAID, ERROR_OWN_RIDE_PAYMENT,
                        ERROR_PAYMENT_PENDING, ERROR_PAYMENT_PROCESSING,
                        ERROR_PERMISSION_DENIED, INFO_PAYMENT_CANCELLED,
                        INFO_PAYMENT_VERIFYING, PAYMENT_METHOD_STRIPE,
                        PAYMENT_STATUS_CANCELLED, PAYMENT_STATUS_COMPLETED,
                        PAYMENT_STATUS_FAILED, PAYMENT_STATUS_PENDING,
                        PAYMENT_STATUS_REFUNDED, SUCCESS_PAYMENT_CANCELLED,
                        SUCCESS_PAYMENT_COMPLETED, SUCCESS_PAYMENT_REFUNDED)
from .forms import PaymentForm, PaymentInitForm
...
def payment_list(request):
    ...
    payments_received = Payment.objects.filter(recipient=request.user).order_by(
        "-created_at"
    )

    context = {"payments_made": payments_made, "payments_received": payments_received}
...
def my_payments(request):
    """
    Vista para mostrar todos los pagos relacionados con el usuario actual
    """
    # Obtiene todos los pagos donde el usuario es pagador o receptor
    ...
    # Pagos enviados y recibidos por separado
    payments_sent = payments.filter(payer=request.user)
    payments_received = payments.filter(recipient=request.user)
    ...
    context = {
        ...
        "is_my_payments": True,  # Marca para la plantilla
    }
...
def payment_detail(request, pk):
    ...
    context = {
        ...
        or request.user.is_superuser,  # Flag para ver si es admin
    }
...
def create_payment(request, ride_id):
    ...
    if ride.seats_available <= 0:
        messages.error(
            request, "Este viaje está completo, no hay asientos disponibles."
        )
        return redirect("rides:ride_detail", ride_id=ride_id)
    ...
    return render(request, "payments/create_payment.html", {"form": form, "ride": ride})
        ...
        if status == "succeeded":
            ...
            if payment.ride:
                ...
                if add_passenger_to_ride(payment.payer, payment.ride):
                    ...
                    messages.warning(
                        request,
                        "El pago fue exitoso, pero no se pudo reservar el asiento. Por favor, contacta con soporte.",
                    )
